      ...
      if (mainOpen !== -1 && mainClose !== -1 && mainClose > mainOpen) {
        ...
        if (headings.length) {
          ...
          const tocItems = sections.map(sec => {
            ...
            if (!hasChildren) {
              return `<li${liClass}><a href="#${escapeHtml(sec.id)}">${escapeHtml(sec.title)}</a>${nested}</li>`;
            }
            const title = escapeHtml(sec.title);
            const toggle = `<button class="toc-toggle" aria-label="Collapse ${title}" title="Collapse section" aria-expanded="true"><span class="toc-toggle-icon" aria-hidden="true"></span></button>`;
            return `<li${liClass}><div class="toc-section-row">${toggle}<a class="toc-section-link" href="#${escapeHtml(sec.id)}">${title}</a></div>${nested}</li>`;
          }).join('');
