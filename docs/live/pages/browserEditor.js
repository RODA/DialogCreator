...
(() => {
            ...
            switch (key) {
              ...
              case "lineClamp":
                ...
                if (button) {
                  ...
                  const clamp2 = Number(value) || 1;
                  const maxHeightPx = Math.round(lineHeightPx * clamp2 + padT + padB + borT + borB);
                  host.style.maxHeight = maxHeightPx + "px";
                  if (span2) {
                    span2.style.setProperty("-webkit-line-clamp", String(clamp2));
                  }
                } else if (label) {
                  element.dataset[key] = value;
                  renderutils.updateLabel(element);
      ...
      (function(global, factory) {
        ...
        var IroColor = /* @__PURE__ */ (function() {
          ...
          IroColor2.hsvToRgb = function hsvToRgb(hsv) {
            ...
            return {
              r: clamp2(r2 * 255, 0, 255),
              g: clamp2(g2 * 255, 0, 255),
              b: clamp2(b2 * 255, 0, 255)
            };
          };
          IroColor2.rgbToHsv = function rgbToHsv(rgb) {
            ...
            return {
              h: hue * 60 % 360,
              s: clamp2(saturation * 100, 0, 100),
              v: clamp2(value * 100, 0, 100)
            };
          };
          ...
          IroColor2.kelvinToRgb = function kelvinToRgb(kelvin) {
            ...
            return {
              r: clamp2(floor(r2), 0, 255),
              g: clamp2(floor(g2), 0, 255),
              b: clamp2(floor(b2), 0, 255)
            };
          };
  ...
  // src/editor/editorDocumentController.ts
  function clearDialogCustomState(dialog2) {
    dialog2.customJS = "";
    dialog2.i18n = void 0;
    if (!dialog2.syntax) {
      dialog2.syntax = { command: "", defaultElements: [] };
      return;
    }
    dialog2.syntax.command = "";
  }
  function updateDialogPropertyInput(id, value) {
  ...
  function resetDialogProperties(payload) {
    const data = payload;
  ...
  function clearStalePropertiesPanel(setElementSelected) {
    const propsList = document.getElementById("propertiesList");
  ...
  function installEditorDocumentController(options) {
    const transport3 = options.transport;
    transport3.on("request-dialog-json", () => {
      try {
        transport3.send("send-to", "main", "dialog-json", options.editor.stringifyDialog());
      } catch {
        transport3.send("send-to", "main", "dialog-json", "");
      }
    });
    transport3.on("load-dialog-json", (data) => {
      try {
    ...
    transport3.on("reset-dialog-properties", (payload = {}) => {
      try {
        resetDialogProperties(payload);
      } catch {
      }
    });
    transport3.on("load-dialog-json", () => {
  ...
  // src/editor/editorBootstrap.ts
  function bootEditor(rendererTransport) {
    setRendererTransport(rendererTransport);
    let elements2 = { ...elements };
    Object.keys(elements2).forEach((element) => {
      coms.sendTo("main", "getProperties", element);
    });
    let elementSelected = false;
    let propertyUpdate = function(_ev) {
    };
    const propertyUpdateOnEnter = (ev) => {
      if (ev.key == "Enter") {
    ...
    let ephemeralLeftBlurHandler = null;
    let ephemeralTopBlurHandler = null;
    ...
    coms.on("elementSelected", (id) => {
      ...
      document.getElementById("bringForward").disabled = false;
      document.getElementById("sendBackward").disabled = false;
      const leftInput0 = document.getElementById("elleft");
      const topInput0 = document.getElementById("eltop");
      ...
      if (!propsList) {
        ...
        propsList.dataset.currentElementId = String(id);
      }
      const element = document.getElementById(id);
      const typeName = String(element?.dataset?.type || "").trim();
      if (typeName) {
        const templateKey = `${typeName.charAt(0).toLowerCase()}${typeName.slice(1)}Element`;
        const template = elements2[templateKey];
        const inner = element.firstElementChild;
        if (template) {
          for (const [key, rawValue] of Object.entries(template)) {
            if (key.startsWith("$") || rawValue === void 0 || rawValue === null) continue;
      ...
      ellist.forEach((el) => {
        ...
        if (item.name in dataset) {
          item.disabled = false;
          const row = item.closest(".element-property");
          if (row) row.classList.remove("hidden-element");
          item.dataset.bindElementId = String(id);
          const booleanProps = /* @__PURE__ */ new Set(["isEnabled", "isVisible", "isSelected", "isChecked"]);
      ...
      const alignMiddleBtn = document.getElementById("alignMiddle");
      ...
      const groupBtn = document.getElementById("groupElements");
      const ungroupBtn = document.getElementById("ungroupElements");
    ...
    coms.on("elementSelectedMultiple", (...args) => {
      elementSelected = true;
      const propsList = document.getElementById("propertiesList");
      if (propsList) {
        propsList.dataset.currentElementId = "";
        ...
        const typeInput = document.getElementById("eltype");
        const leftInput = document.getElementById("elleft");
        ...
        if (leftInput) leftInput.removeEventListener("blur", propertyUpdate);
        if (topInput) topInput.removeEventListener("blur", propertyUpdate);
        if (leftInput) leftInput.removeEventListener("keyup", propertyUpdateOnEnter);
        if (topInput) topInput.removeEventListener("keyup", propertyUpdateOnEnter);
        ...
        if (leftInput) {
          ...
          leftInput.addEventListener("keyup", propertyUpdateOnEnter);
        }
        if (topInput) {
          ...
          topInput.addEventListener("keyup", propertyUpdateOnEnter);
        }
      }
      document.getElementById("bringToFront").disabled = false;
      document.getElementById("sendToBack").disabled = false;
      ...
      const alignLeftBtn = document.getElementById("alignLeft");
      const alignTopBtn = document.getElementById("alignTop");
      const alignMidBtn = document.getElementById("alignMiddle");
      const alignRightBtn = document.getElementById("alignRight");
      const alignBottomBtn = document.getElementById("alignBottom");
      const alignCenterBtn = document.getElementById("alignCenter");
      const refreshAlign = () => {
    ...
    window.addEventListener("DOMContentLoaded", async () => {
      const { editor: editor2 } = await Promise.resolve().then(() => (init_editor(), editor_exports));
      propertyUpdate = editor2.propertyUpdate;
      ...
      await editor2.initializeDialogProperties();
      await editor2.makeDialog();
      await editor2.addAvailableElementsTo("editor");
      await editor2.addDefaultsButton();
      ...
      const sendJsonToMain = () => {
        try {
          coms.sendTo("main", "document-json-updated", editor2.stringifyDialog());
        } catch {
        }
      };
      ...
      try {
        const props = document.querySelectorAll('#dialog-properties [id^="dialog"]');
        props.forEach((input) => {
      ...
      const codeBtn = document.getElementById("dialog-code");
      if (codeBtn) {
        codeBtn.disabled = false;
        codeBtn.addEventListener("click", async () => {
          const json = editor2.stringifyDialog();
      ...
      document.getElementById("removeElement")?.addEventListener("click", editor2.removeSelectedElement);
      const setDialogCustomJS = function(text) {
      ...
      window.dialogCreatorSetCustomJS = setDialogCustomJS;
      coms.on("setDialogCustomJS", setDialogCustomJS);
      document.getElementById("bringToFront")?.addEventListener("click", editor2.bringSelectedToFront);
      document.getElementById("sendToBack")?.addEventListener("click", editor2.sendSelectedToBack);
      document.getElementById("bringForward")?.addEventListener("click", editor2.bringSelectedForward);
      document.getElementById("sendBackward")?.addEventListener("click", editor2.sendSelectedBackward);
      document.getElementById("alignLeft")?.addEventListener("click", () => editor2.alignSelection("left"));
      document.getElementById("alignTop")?.addEventListener("click", () => editor2.alignSelection("top"));
      document.getElementById("alignMiddle")?.addEventListener("click", () => editor2.alignSelection("middle"));
      document.getElementById("alignRight")?.addEventListener("click", () => editor2.alignSelection("right"));
      document.getElementById("alignBottom")?.addEventListener("click", () => editor2.alignSelection("bottom"));
      document.getElementById("alignCenter")?.addEventListener("click", () => editor2.alignSelection("center"));
      document.getElementById("groupElements")?.addEventListener("click", () => editor2.groupSelection());
      document.getElementById("ungroupElements")?.addEventListener("click", () => editor2.ungroupSelection());
      ...
      document.addEventListener("keydown", function(ev) {
        ...
        const selectedEls = Array.from(document.querySelectorAll("#dialog .selectedElement"));
        ...
        if (selectedEls.length > 1) {
          ...
          if (Object.keys(props).length) {
            renderutils.updateElement(el, props);
            if (props.left !== void 0) el.dataset.left = String(props.left);
            if (props.top !== void 0) el.dataset.top = String(props.top);
          }
        }
        ev.preventDefault();
      });
      ...
      document.addEventListener("keydown", function(ev) {
        ...
        if (isKeyG) {
          ev.preventDefault();
          if (ev.shiftKey) {
            editor2.ungroupSelection();
          } else {
            editor2.groupSelection();
          }
          return;
        }
        const keyCode = ev.keyCode;
        ...
        if (isArrowUp && ev.shiftKey) {
          ev.preventDefault();
          editor2.bringSelectedToFront();
        } else if (isArrowDown && ev.shiftKey) {
          ev.preventDefault();
          editor2.sendSelectedToBack();
        } else if (isArrowUp) {
          ev.preventDefault();
          editor2.bringSelectedForward();
        } else if (isArrowDown) {
          ev.preventDefault();
          editor2.sendSelectedBackward();
        }
      });
      document.addEventListener("keydown", function(ev) {
        const metaOrCtrl = ev.metaKey || ev.ctrlKey;
      ...
      coms.on("propertiesFromDB", (...args) => {
        ...
        if (pkeys.length > 0) {
          for (const pkey of pkeys) {
            ...
            if (utils.isKeyOf(elements2, name)) {
              const element = elements2[name];
              const k = pkey;
              if (k in element) {
                element[k] = value;
              }
      ...
      installEditorDocumentController({
      ...
      window.dialogCreatorEditorActions = {
        getDialogJson() {
          return editor2.stringifyDialog();
        },
        loadDialogJson(data) {
          editor2.loadDialogFromJson?.(data);
          setTimeout(() => {
            try {
              scheduleJsonUpdate();
            } catch {
        ...
        newDialog() {
          ...
          const setDialogInput = function(id, value) {
            const input = document.getElementById(id);
            if (!input) return;
            input.value = value;
            input.dispatchEvent(new Event("change", { bubbles: true }));
          ...
          setDialogInput("dialogName", "NewDialog");
          setDialogInput("dialogTitle", "New dialog");
          setDialogInput("dialogLanguage", "en_US");
          setDialogInput("dialogRuntimeProvider", "R");
          setTimeout(() => {
            try {
              scheduleJsonUpdate();
            } catch {
  ...
  // src/shell-web/browserEditor.ts
  var transport2 = createBrowserRendererTransport();
  var catalog = createStaticElementCatalog();
  var browserPanelStyleId = "dialogcreator-browser-panel-styles";
  var closeCurrentBrowserPanel = null;
  function recordBrowserEditorEvent(event) {
    const target = window;
    const events = target.dialogCreatorBrowserEditorEvents || [];
    events.push(event);
    target.dialogCreatorBrowserEditorEvents = events;
  }
  function customJSFromPanelData(data) {
    if (typeof data !== "string") {
      return "";
    }
    try {
      return String(JSON.parse(data).customJS || "");
    } catch {
      return "";
    }
  }
  ...
  function installBrowserPanelStyles() {
  ...
  function createBrowserPanelCloseButton() {
    const button = document.createElement("button");
    button.type = "button";
  ...
  function centerBrowserPanelWindow(panel, panelWindow) {
  ...
  function installBrowserPanelDrag(panel, panelWindow, titlebar) {
  ...
  function openBrowserPanel(args) {
    closeBrowserPanel();
  ...
  function sendBrowserPanelPayload(frame, args) {
  ...
  function sendBrowserShortcut(command) {
  ...
  function installBrowserShortcuts() {
    window.addEventListener("keydown", (event) => {
      ...
      if (key === "p") {
        event.preventDefault();
        const actions = window.dialogCreatorEditorActions;
  ...
  function sendPanelMessageUntilApplied(frame, type, data) {
    const requestId = `panel-payload-${Date.now()}-${Math.random().toString(16).slice(2)}`;
    let attempts = 0;
    let applied = false;
    const onMessage = function(event) {
      const message = event.data;
      if (message?.type === "dialogcreator-panel-payload-applied" && message.requestId === requestId) {
        applied = true;
  ...
  window.addEventListener("message", (event) => {
    ...
    if (message?.type === "dialogcreator-code-saved") {
      ...
      window.dialogCreatorSetCustomJS?.(code);
      transport2.emit("setDialogCustomJS", code);
      transport2.emit("message-from-main-setDialogCustomJS", code);
      return;
    }
  ...
  transport2.on("send-to", async (windowName, channel, ...args) => {
    ...
    if (channel === "secondWindow") {
      openBrowserPanel(args[0]);
      return;
    }
    if (channel === "close-secondWindow" || channel === "close-codeWindow" || channel === "close-previewWindow") {
      closeBrowserPanel();
    }
  });
  bootEditor(transport2);
