...
Some properties can be overwritten with custom JavaScript code in the Actions window, for example Enable or Visible, using a pre-defined set of API commands, see the details in the [API reference](API.md).
...
Dialog Creator supports custom JavaScript through a built-in API. For events, helper functions, and the full reference, open the [API reference](API.md).
